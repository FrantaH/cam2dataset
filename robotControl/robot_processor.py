...
class RobotProcessor:
    ...
    def __init__(self, ip="192.168.1.6"):
        '''
        inicialize robot object for later communication with the robotic arm
        open connection and set some basic and default variables
        '''
        ip = ip
        ...
        try:
            self.dashboard_handle = DobotApiDashboard(
                ip, dashboard_port, self.text_log)
            self.move_handle = DobotApiMove(
                ip, move_port, self.text_log)
        ...
        self.connected = True
        time.sleep(0.1)
        self.dashboard_handle.EnableRobot()

        self.dashboard_handle.AccJ(80)
        self.dashboard_handle.AccL(80)
        self.dashboard_handle.SpeedJ(100)
        self.dashboard_handle.SpeedL(100)
    ...
    def __del__(self):
        ...
        self.dashboard_handle.DisableRobot()
        time.sleep(0.5)
        
        self.connected = False
        self.dashboard_handle.close()
        self.move_handle.close()
        self.feed_handle.close()

    def reset(self):
        self.dashboard_handle.ClearError()
        self.stop_move()
        time.sleep(5)
        self.__init__()
    ...
    def move_to_coors(self,x,y,z,r=-100):
        self.move_handle.MovJ(x,y,z,r)

    def get_draging_points(self):
        points = []
        self.dashboard_handle.StartDrag()

        while(True):
            key = input("Press Enter to record coordinate, type r to return points, type n to start over:")
            if 'r' in key.lower():
                break
            if 'n' in key.lower():
                points = []
                print("all points thrown away")
                continue
            print("robot coordinates are:", self.feed["tool_vector_actual"][0][:4])
            points.append(self.feed["tool_vector_actual"][0][:4])
    ...
    def get_DI(self, idx, get_type = "feed"):
        if get_type == "query":
            ...
            return self.feed["digital_input_bits"][0] & (1 << idx-1) > 0

    def set_DO(self, idx, queue = False, value = 1):
        if isinstance(idx, list):
            idx = [[item, value] for item in idx]
            print(np.ravel(idx))
            self.dashboard_handle.DOGroup(np.ravel(idx))
        elif isinstance(idx, str):
            if idx == "all":
                idxs = np.ravel([(x+1,value) for x in range(16)])
                self.dashboard_handle.DOGroup(idxs)
        else:                
            if queue:
                self.dashboard_handle.DO(idx,value)
            else:
                self.dashboard_handle.DOExecute(idx,value)

    def unset_DO(self, idx, queue = False):
        self.set_DO(idx,queue=queue,value=0)
    
    def toggle_DO(self, idx, queue = False):
        self.set_DO(idx,queue=queue,value=self.feed)

    def mov(self, point, *args, mtype = "movj"):
        if len(point) == 0:
            self.stop_move()
        elif len(point) == 3:
        ...
        if mtype == "movj":
            self.move_handle.MovJ(point[0],point[1],point[2],point[3], *args)
        elif mtype == "movl":
    ...
    def move_path(self, points, *args, mtype = "movj", CP = 1):
        # if self.feed['speed_scaling'] > 40:
        #     key = input("Speed of robot is pretty high (" + str(self.feed['speed_scaling'][0]) + "), are you sure you want to continue? Press Enter to continue, type n to skip this step:")
        #     if 'n' in key.lower():
        #         return
        self.dashboard_handle.SpeedJ(30)
        self.dashboard_handle.SpeedL(30)
        self.dashboard_handle.CP(CP)
        for point in points:
            self.mov(point, *args, mtype=mtype)
        self.dashboard_handle.SpeedJ(100)
        self.dashboard_handle.SpeedL(100)
    ...
    def mirror(self):
        ...
        try:
            while True:
                print("press ctrl+c to end the move loop")
                self.move_path(points)
                time.sleep(0.1)
        except KeyboardInterrupt:
            self.move_handle.MoveJog()

    def dump_feed(self):
        for field_name in MyType.names:
            field_value = self.feed[field_name][0]
            field_type = MyType.fields[field_name][0]
            print(f"{field_name} ({field_type}) : {field_value}")

    def move_testing(self):
        self.dashboard_handle.SpeedFactor(100)
        points = np.array([[-168.0, 263.0, -20.0, -17.0], [147.0, 244.0, -10.0, -59.0]])
        sensor_id = 3
        sensor_stop_state = False
        # first try fast -> slow move
        # nejdřív zkusit pomocí accj
        vector = points[1]-points[0]
        self.dashboard_handle.AccL(1)
        self.mov(points[0], mtype="movl")
        self.mov(points[1], mtype="movl")
        self.mov(points[0], mtype="movl")
        self.mov(points[1], mtype="movl")
        self.dashboard_handle.AccL(100)


        # druhej pokus přes změnu rychlosti během pohybu
        continuous_points = [points[0], points[0] + vector/4, points[0] + 2*vector/4, points[0] + 3*vector/4, points[0] + vector]
        for i, point in enumerate(continuous_points):
            speed = int(100 - (i-1)*100/(len(continuous_points)-2))  #int((i+1)*(100/4))
            speed = max(speed,1)
            speed = min(speed,100)
            speed_arg = "SpeedL="+str(speed)
            self.mov(point, speed_arg, mtype="movl")#, mtype="movj")
        for i, point in enumerate(continuous_points):
            speed = int(100 - (i-1)*100/(len(continuous_points)-2))  #int((i+1)*(100/4))
            speed = max(speed,1)
            speed = min(speed,100)
    ...
    def pick_up(self, DI, DI_goal_status, minZ, point = None):
        if point == None:
            ...
            point = self.feed["tool_vector_actual"][0][:4]
            point[2] = minZ
        self.move_until_DI(point, DI, DI_goal_status)
        
    def move_until_DI(self, point, DI, DI_goal_status):
        # self.dashboard_handle.SpeedFactor(50)
        self.dashboard_handle.CP(1)
        if isinstance(point, list) or isinstance(point, np.ndarray):
            self.mov(point,"SpeedL=50", mtype="movl")
        elif isinstance(point, str):
            self.move_handle.MoveJog(point)
            print("-------------------------")
            print("ALERT: hazard of crash!!!")
            print("-------------------------")
        else:
            print("wrong point argument")
            return
        self.wait_DI(DI, DI_goal_status, end_point = point)
        self.stop_move()
    ...
    def feed_back(self):
        ...
        while True:
            ...
            data = bytes()
            while hasRead < 1440:
                temp = self.feed_handle.socket_dobot.recv(1440 - hasRead)
                if len(temp) > 0:
                    hasRead += len(temp)
                    data += temp
            hasRead = 0

            self.feed = np.frombuffer(data, dtype=MyType)
            # print("robot_mode:", self.feed["robot_mode"][0])
            # print("test_value:", hex((self.feed['test_value'][0])))
            if hex((self.feed['test_value'][0])) == '0x123456789abcdef':

                # check alarms
                if self.feed["robot_mode"] == 9:
...
"""
    ...
    def set_button_move(self, master, text, rely, x, **kargs):
        self.button = Button(master, text=text, padx=5)
        self.button.bind("<ButtonPress-1>",
                         lambda event: self.move_jog(text=text))
        self.button.place(rely=rely, x=x)
    ...
    def set_button_stopMove(self, master, text, rely, x, **kargs):
        self.button = Button(master, text=text, padx=5)
        self.button.bind("<ButtonPress-1>", self.move_stop)
        self.button.place(rely=rely, x=x)
    ...
    def set_feed(self, text_list, x1, x2, x3, x4):
        ...
        self.set_button_move(
            self.frame_feed, text_list[2][0], rely=0.2, x=x4, command=lambda: self.move_jog(text_list[2][0]))
        self.set_button_stopMove(
            self.frame_feed, text_list[2][0], rely=0.25, x=x4, command=lambda: self.move_jog(text_list[2][0]))
        self.set_button_bind(
        ...
        # self.set_button_move(
        #     self.frame_feed, text_list[2][3], rely=0.6, x=x4, command=lambda: self.move_jog(text_list[2][0]))
        # self.set_button_stopMove(
        #     self.frame_feed, text_list[2][3], rely=0.7, x=x4, command=lambda: self.move_jog(text_list[2][0]))
